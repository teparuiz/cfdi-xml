...
const xmlTimbrado = xmlDoc.getElementsByTagName("tfd:TimbreFiscalDigital");
    ...
    let TotalTrasladosImpuestoIVA16: number = 0;
    let TotalTrasladosBaseIVA16: number = 0;
    let MontoTotalPagos: number = 0;
    let TotalRetencionesIVA: number = 0;
    let TotalRetencionesIEPS: number = 0;
    let TotalRetencionesISR: number = 0;
    ...
    this.#pagos.forEach((item) => {
      ...
      item.doctoRelacionado.forEach((item) => {
        ...
        doctoNode.setAttribute(
          "EquivalenciaDR",
          item.EquivalenciaDR.toString()
        );
        doctoNode.setAttribute(
          "NumParcialidad",
          item.NumParcialidad.toString()
        );
        ...
        doctoNode.setAttribute(
          "ImpSaldoInsoluto",
          item.ImpSaldoInsoluto.toFixed(2).toString()
        );
        ...
        item.ImpuestosDR.forEach((item) => {
          if (item.Tipo === "traslado") {
            ...
            traslado_dr.setAttribute(
              "TipoFactorDR",
              item.TipoFactorDR.toString()
            );
            traslado_dr.setAttribute(
              "TasaOCuotaDR",
              item.TasaOCuotaDR.toFixed(6).toString()
            );
        ...
        item.ImpuestosDR.forEach((item) => {
          if (item.Tipo === "traslado") {
            ...
            if (item.ImpuestoDR === "002") {
              ...
              totalesNode.setAttribute(
                "TotalTrasladosBaseIVA16",
                TotalTrasladosBaseIVA16.toFixed(2).toString()
              );
              totalesNode.setAttribute(
                "TotalTrasladosImpuestoIVA16",
                TotalTrasladosImpuestoIVA16.toFixed(2).toString()
              );
          ...
          if (item.Tipo === "retencion") {
            ...
            totalesNode.setAttribute(
              "TotalRetencionesIVA",
              TotalRetencionesIVA.toString()
            );
            totalesNode.setAttribute(
              "TotalRetencionesIEPS",
              TotalRetencionesIEPS.toString()
            );
            totalesNode.setAttribute(
              "TotalRetencionesISR",
              TotalRetencionesISR.toString()
            );
