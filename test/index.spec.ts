...
const keyStream = fs.readFileSync(
  "test/CSD_Sucursal_1_EKU9003173C9_20230517_223850.key"
);
const password = "12345678a";

const cerStream = fs.readFileSync(
  "test/CSD_Sucursal_1_EKU9003173C9_20230517_223850.cer"
);
...
describe("Pago", () => {
  ...
  it("Complemento Pago solo traslados", async (): Promise<any> => {
    const payments = [
      {
        payment_form: "03",
        total_amount: 100,
    ...
    const addPayments = payments.map((item) => ({
      ...
      doctoRelacionado: item.payment_related_documents.map((doc) => {
        ...
        if (doc.taxability === "02" && doc.taxes && doc.taxes.length > 0) {
          return {
            ...baseDoc,
            ImpuestosDR: doc.taxes.map(
              (tax: {
                withholding: boolean;
                base: number;
                type: string;
    ...
    const xml = await pago20.generarXmlSellado();


    console.log('---> solo traslados', xml)
  });
  it("Complemento Pago solo traslados", async (): Promise<any> => {
    const payments = [
      {
    ...
    console.log('---> doctos relacionados', xml)
  });

  it("Complemento Pago sin impuestos", async (): Promise<any> => {
    const payments = [
      {
        payment_form: "03",
        total_amount: 6950,
